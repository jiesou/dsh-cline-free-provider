...
/** Ladder of pi-ai effort ids, minus `off` (which is never a real choice here). */
...
export async function fetchFreeModels(url = 'https://api.cline.bot/api/v1/ai/cline/models', fetchImpl = fetch) {
    ...
    for (const raw of payload.data) {
        ...
        const supportedParameters = Array.isArray(raw.supported_parameters)
            ? raw.supported_parameters.filter((x) => typeof x === 'string')
            : undefined;
        models.push({
            id: raw.id,
            ...(name === undefined ? {} : { name }),
...
export async function fetchOpenRouterReasoning(url = 'https://openrouter.ai/api/v1/models', fetchImpl = fetch) {
    ...
    for (const raw of payload.data) {
        ...
        if (efforts === undefined || efforts.length === 0)
            continue;
        byId.set(raw.id, { supportedEfforts: efforts });
    }
    return byId;
}
...
function reasoningLevelsFor(model) {
    ...
    if (efforts === undefined)
        return [];
    return LADDER.filter(level => efforts.includes(level));
}
// `off` is pinned to null — never a string (pi-ai sends that on the *no
// selection* path, hijacking Default into a fixed depth), never absent (that
// offers an "Off" entry sending exactly what Default already sends).
...
function buildModels(scanned, baseURL, config) {
...
const api = {
    stream: (model, context, options) => sanitizeStream(baseApi.stream(model, normalizeReasoningContext(context), options)),
    streamSimple: (model, context, options) => sanitizeStream(baseApi.streamSimple(model, normalizeReasoningContext(context), options)),
};
export async function apply(ctx, config) {
    ...
    // The catalog rotates upstream, so scan on mount and refresh slowly. Mount
    // never awaits it: an unreachable upstream must not kill the plugin.
    async function sync() {
        const [entries, reasoningById] = await Promise.all([
            ...
            fetchOpenRouterReasoning().catch((error) => {
                ctx.logger.warn('[%s] OpenRouter reasoning scan failed; falling back to Cline-only metadata: %s', name, errorChain(error));
                return new Map();
            }),
        ]);
    ...
    const timer = setInterval(() => {
        void sync().catch((error) => {
            ctx.logger.warn('[%s] catalog refresh failed: %s', name, errorChain(error));
        });
    }, REFRESH_MS);
    timer.unref?.();
